...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="colorhug dell doc +gpg +man nvme pkcs7 redfish systemd elogind test thunderbolt uefi"
REQUIRED_USE="
	${PYTHON_REQUIRED_USE}
	dell? ( uefi )
"
...
src_configure() {
	...
	local emesonargs=(
		...
		-Dconsolekit="$(usex systemd false $(usex elogind false true))"
...
src_install() {
	...
	if ! use systemd ; then
		if use elogind ; then
			newinitd "${FILESDIR}"/${PN}-elogind fwupd
		else
			newinitd "${FILESDIR}"/${PN}-consolekit fwupd
		fi
