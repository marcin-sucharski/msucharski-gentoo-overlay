...
then
	...
	then
		...
		SRC_URI="
		!custom-jdk? ( https://download-cf.jetbrains.com/idea/${MY_PN}IU-${MY_PV_SRC}-no-jdk.tar.gz -> ${MY_PN}IU-${PV_STRING}-no-jdk.tar.gz )
		custom-jdk? ( https://download-cf.jetbrains.com/idea/${MY_PN}IU-${MY_PV_SRC}.tar.gz -> ${MY_PN}IU-${PV_STRING}.tar.gz )
		"
	else
		SRC_URI="
